									...
									<c:when test="${not empty qnaList}">
										<c:forEach items="${qnaList }" var="qna">
											<tr>
												...
												<td><a href="${qnaURL }"><c:out value="${qna.qnaTitle }" /></a></td>
												<td><c:out value="${qna.qnaWriter }" /></td>
												<td><c:out value="${qna.qnaDate }" /></td>
												<td><c:if test="${qna.qnaAnswer eq 1}"> <strong>답변완료</strong> </c:if> 
													<c:if test="${qna.qnaAnswer eq 0}"> <span>접수중</span> </c:if></td>
											</tr>
										</c:forEach>
									</c:when>
									<c:otherwise>
...
<form:form id="searchForm" modelAttribute="simpleCondition" action="${cPath }/qna" method="get">
