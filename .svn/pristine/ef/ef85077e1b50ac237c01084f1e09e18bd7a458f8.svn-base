...
function createCalendar(calendarEl){
	var fullCalendar = new FullCalendar.Calendar(calendarEl, {
		  ...
		  eventClick: function(arg) {
				 var updateFormUrl = "${cPath}/com/schedule/"+ arg.event.id +"/form";
				 $.ajax({
		    			type: "get",
	      ...
	      events:function(){
	    	  var listResult = $.ajax({
	    			type: "POST",
	    			url: "${cPath}/com/schedule/list",	// form을 전송할 실제 파일경로
	    		    dataType : "json", 
...
// 	console.log(data);
	...
	$.ajax({
		type: "POST",
		url: "${cPath}/com/schedule",	// form을 전송할 실제 파일경로
		data: data,
	    dataType : "text", 
		beforeSend : function(xhr) { // 전송 전 실행 코드
			xhr.setRequestHeader(header,token);
...
yu_updateForm.on("submit",function(){
	...
	var updateUrl = "${cPath}/com/schedule/"+ $('#up_scheNo').val() +"/form";
...
//         text: "",
        ...
        if (result.isConfirmed) { //확인
        	...
        	var updateUrl = "${cPath}/com/schedule/"+ data;
        	$.ajax({
        		type: "DELETE",
        		url: updateUrl,	// form을 전송할 실제 파일경로
//         		data: data,
