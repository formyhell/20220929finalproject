...
<div id="services-section" class="pt-80 pt-xs-60" style="padding-top: 5vh">
	<div class="container">
		<div class="row ">
			<div class="col-sm-12">
				<div class="block-title v-line mb-45">
					...
					<p class="italic">원하는 프로젝트에 지원해보세요</p>
				</div>
			</div>
		</div>
		...
		<div class="form-field">
			...
			<div class="project-category">
				...
				<c:choose>
					<c:when test="${authMember.realMember.memType.get(0) eq 'ROLE_COMMEM'}">
						<div>
						<button style="float: right;" class="btn-text" onclick="location='${cPath }/outsourcing/form'">등록하기</button>
						</div>
					</c:when>
					<c:otherwise>
						<div>
						<button style="float: right;" class="btn-text" onclick="location.href='${cPath}/gen/myinfo/recommendProject'">프로젝트 추천</button>
						</div>
					</c:otherwise>
				</c:choose>
				
				<div id="searchUI" class="form-field" style="display: none;">
					<table class="table table-category">
						<tr>
							<th class="gray-bg">제목</th>
							<td><input style="border: 1px solid #DEDEDE; padding: 3px 5px;" placeholder="Search Here" name="projTitle" /> 
								<input type="button" id="searchBtn" class="btn-greenlight" style="width:100px; height:25px;" value="검색">
							</td>
						</tr>
						<tr>
							<th class="gray-bg">고용형태</th>
							<td colspan="2"><input type="checkbox" name="projEmptype"
								class="empType" value="out" id="OUT"> <label for="out">
									외주(도급)</label> <input type="checkbox" name="projEmptype"
								class="empType" value="in" id="IN"> <label for="in">
									상주(기간제)</label></td>
						</tr>
						<tr>
							<th class="gray-bg">직무분야</th>
							<td colspan="2"><input type="checkbox" name="projObj"
								class="obj" value="WEBDESIGN" id="WEBDESIGN"> <label
								for="webdesign"> 웹디자인</label> <input type="checkbox"
								name="projObj" class="obj" value="PUBLISHER" id="PUBLISHER">
								<label for="publisher"> 퍼블리셔</label> <input type="checkbox"
								name="projObj" class="obj" value="FRONTEND" id="FRONTEND">
								<label for="frontend"> 프론트엔드</label> <input type="checkbox"
								name="projObj" class="obj" value="BACKEND" id="BACKEND">
								<label for="backend"> 백엔드</label> <input type="checkbox"
								name="projObj" class="obj" value="PLANNER" id="PLANNER">
								<label for="planner"> 기획자</label></td>
						</tr>
						<tr>
							<th class="gray-bg">기업 등급</th>
							<td colspan="2"><input type="checkbox" name="comGrd"
								class="grd" value="gold" id="GOLD"> <label for="gold">골드</label>
								<input type="checkbox" name="comGrd" class="grd" value="silver"
								id="SILVER"> <label for="silver">실버</label> <input
								type="checkbox" name="comGrd" class="grd" value="bronze"
								id="BRONZE"> <label for="bronze"> 브론즈</label> <input
								type="checkbox" name="comGrd" class="grd" value="common"
								id="COMMON"> <label for="common">일반</label></td>
						</tr>
						<tr>
							<th class="gray-bg">진행기간</th>
							<td>시작일 <input type="date" id="sdate" name="projSdate">
								~ 종료일 <input type="date" id="edate" name="projFdate">
							</td>
						</tr>
						<tr>
							<th class="gray-bg">필요 기술</th>
							<td><c:forEach items="${outSkillList}" var="skill"
									varStatus="i">
									<input type="checkbox" name="projSkill" id="${skill.codeId }"
										value="${skill.codeId }" class="skill" />
												...
												<div class="card about-block dark-gray-bg clearfix" style="min-height: 250px; padding-right:20px; padding-left:20px">
													<div class="text-box mt-25">
														...
														<div class="text-content">
															<p>
																~ ${proj.projRecruitfdate} 까지<br>${proj.projEmptype} | ${proj.projEstimate}원 | 등록일 : ${proj.projRecruitsdate }
															</p>
															<p style="max-height:50px;" class="ellipsis">${proj.projContent}<br> <span class="tooltiptext tooltip-bottom">${proj.projContent}</span> </p>
														</div>
													</div>
												</div>
...
<form:form id="searchForm" modelAttribute="detailCondition"
	action="${cPath }/outsourcing/listAll" method="get">
