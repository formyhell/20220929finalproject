...
<%-- 								<c:if test="${authMember.realMember.memType.get(0) eq 'ROLE_GENMEM' }"> --%>
									<c:if test="${cmnt.cmntWriter eq authMember.realMember.memId }">
										<button onclick="location='${cPath }/community/${cmnt.cmntNo }/form'" class="btn btn-blueOcean">수정</button>
										<form:form action="${cPath }/community/${cmnt.cmntNo}" method="post" modelAttribute="cmnt" class="i-block" id="delForm">
											<input type="hidden" name="_method" value="delete" id="delForm" />

											<input type="submit" value="삭제" data-href="${cPath }/community/${cmnt.cmntNo }" class="btn btn-grayDay" id="delBtn"/>
...
<%-- 								</c:if> --%>
						...
						<div class="text-center">
							<button id="cmntRec" class="line-btn"  style="display: inline-block;" ><i class="fa fa-thumbs-up"></i>추천<span id="recRst">${cmnt.cmntRec }</span></button>
							<button id="cmntReport" class="line-btn" onclick="f_modalOpen()"  style="display: inline-block;"><i class="fa fa-bell"></i>신고<span id="repRst">${cmnt.cmntReport }</span></button>
						</div>
					<hr />
					</div>
							...
							<form method="post" id="repInput">
								...
								<div class="mtb-60">
									<div class="row mt-30">
										<div>
											<div class="col-md-12">
												<input type="hidden" id="repCmntno" name="repCmntno" value="${cmnt.cmntNo }"> 
		...
		<div class="modal-content padding-20 width-75per" >
			<div class="modal-header">
				<h5 class="modal-title">
					신고 사유<input type="checkbox" name="checkbox" id="checkbox" onchange="check1(this)">
...
var repWriter = "${authMember.realMember.memId }";
// <c:if test="${authMember.realMember.memType.get(0) eq 'ROLE_COMMEM' }">
	...
	$.ajax({
		...
		success: function(result) {
			if(result == "추천되었습니다"){
				...
				$("#cmntRec").addClass("active");
				successAlert(result);
			} else if(result == "추천 취소되었습니다"){
				$("#recRst").text(Number(cmntRec)-1);
				$("#cmntRec").removeClass("active");
				infoAlert(result);
			}
		...
		, success: function(result) {
			...
			if(result.repList.length == 0){
				...
				if(rep.genImg != null){
					...
					img = "${cPath}/resources/images/" + rep.comImg; 
				} else {
					img = "${cPath}/resources/assetsPms/images/dashboard/1.png";
				}
					replyView.append(
...
function check1(f){
    if (f.checked) {
      $('#trepReason').val('자유 게시판에 어울리지 않은 글입니다 삭제 부탁드립니다.');
      }
    }
</script>