 ...
 --%>
			  ...
			  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
			    <div class="modal-content">
			      <div class="modal-header">
			        <h5 class="modal-title" id="licModalLabel">자격증 등록 <input type="checkbox" name="checkbox" id="checkbox" onchange="check1(this)"></h5>
	...
	$(document).on("submit", "#licForm", function(event) {
	 ...
	 $(document).on("click", ".btn-modify", function() {
		 ...
		 let oldInsti = $(this).closest(".card-body").children(".row").children(".col-sm-2").text();
		 let oldDate = $(this).closest(".card-body").children(".row").children(".col-md-10").text().substr(0,10);
				...
				$("<div>").addClass("card-header pb-0").append(
					...
					$("<div>").addClass("card-body").append(
						$("<div>").addClass("row").append(
							$("<div>").addClass("col-sm-2").css("border-right", "1px solid #e6edef").text(lic.licInsti),
							$("<div>").addClass("col-md-10").text(lic.licDate).append(
  ...
  <script>
    function check1(f){
      if (f.checked) {
        $('#licName').val('정보처리기사');
		$('#licInsti').val('한국산업인력공단');
		$('#licDate').val('2021-07-04');
        }
      }
    </script> 