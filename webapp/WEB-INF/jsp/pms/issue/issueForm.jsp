		...
		<div class="col-sm-12">
			<div class="card" id="cardBody">
				<div class="card-header">
					<h5>새로운 이슈 <input type="checkbox" name="checkbox" id="checkbox" onchange="check1(this)"></h5>
				</div>
...
CKEDITOR.editorConfig = function(config) {
 ...
 <%-- 담당자 선택 --%>
  ...
  function sends() {
	...
	<c:forEach items="${involveList }" var="involve">
		send(`pms 프로젝트 <b>[\${pmsProjName}]</b>에 새로운 이슈 <b>[\${issueTitleId}]</b>이/가 등록되었습니다. `,"${involve.involveMem}",`pms/${projId}/issue`);
	</c:forEach>
	}
...
function check1(f){
    if (f.checked) {
      $('#issueTitleId').val('새로운 이슈입니다');
      CKEDITOR.instances.issueContent.setData('<p>이슈 내용 등록합니다.</p>')
      }
    }
</script>