...
public class AwardController {
	...
	@ResponseBody
	public AwardVO updateAward(
			@Validated(UpdateGroup.class) @RequestBody AwardVO award
			, Errors errors
			, Authentication authentication 
			) {
		boolean valid =  !errors.hasErrors();
		AwardVO newAward = null;
		if(valid) {
			MemberPrincipal userPrincipal = (MemberPrincipal) authentication.getPrincipal();
			MemberVO member = userPrincipal.getRealMember();
			String memId = member.getMemId();
			award.setAwardMem(memId);
			newAward = service.update(award);
		}
		return newAward;
	}
