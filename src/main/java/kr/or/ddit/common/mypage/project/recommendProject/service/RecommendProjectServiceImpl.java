	...
	public List<ProjectVO> selectProjectList(PagingVO<ProjectVO> pagingVO, String genId) {
		...
		if(list.size()==0) {
			...
			for (ProjectVO vo : finalList) {
				...
				vo.setProjContent(vo.getProjContent().replaceAll(">", "&gt;"));
				if(vo.getProjQualifi().length() > 60) {				
					vo.setProjSubQualifi(vo.getProjQualifi().substring(0,50)+"...");
				}else {
					vo.setProjSubQualifi(vo.getProjQualifi());
				}
				if(vo.getProjContent().length() > 60) {
					vo.setProjSubContent(vo.getProjContent().substring(0,50)+"...");
				}else {
					vo.setProjSubContent(vo.getProjContent());
				}
			}
