...
//		log.info("issueList : {}",issueList);
	...
	public ServiceResult modifyIssue(IssueVO issue) {
		...
		int rowcnt = dao.updateIssue(issue);
	...
	public List<IssueVO> involveIssueList(PagingVO<IssueVO> pagingVO) {
		pagingVO.setTotalRecord(dao.involveIssueTotal(pagingVO));
		List<IssueVO> involveIssueList = dao.involveIssueList(pagingVO);
		pagingVO.setDataList(involveIssueList);
		
		return involveIssueList;
	}
